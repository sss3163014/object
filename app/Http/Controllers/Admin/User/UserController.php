...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        if($request->has('keywords')) {
           ...
           $list =\DB::table("data_users_info")->where('nickname','like','%'.$input.'%')->paginate(3);
           return view('admin.user.list',['users' => $list, 'keys' =>$input ]); 
        }else{
            $input = '';
            $list = \DB::table('data_users_info')->paginate(3);
            return view('admin.user.list',['users' => $list, 'keys' =>$input ]); 
        }
    ...
    {
        $input = $request ->all();
        $rr = DB::table('index_users_login')->where('login_name',$input['name'])->get()->first();
        if (!empty($rr)) {
          return back()->with('mes','账号已存在~');
        } 
        $ra = DB::table('data_users_register')->where('tel',$input['phone'])->get()->first();
      if (!empty($ra)) {
          return back()->with('mes','手机已经注册~');
        } 
      ...
      if($res && $re && $re1) {
        return view('admin.user.add')->with('mes','添加成功');
      } else{
...
//         3 执行授权操作
        ...
        return redirect('admin/user')->with('mes','修改成功~');
...
//        4 授权成功或失败的处理
    ...
    {
      $name = index_users_login::where('user_id',$id)->first()->login_name;
      $user = data_users_info:: where('user_id',$id)->first(); 

      return view('admin/user/user_info',compact('name','user'));
    }

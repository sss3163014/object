...
@foreach($posts as $v)
<li>
<a target="_blank" href="{{ url('home/postshow')}}?id={{ $v->id }}"><img src="{{ asset('homes/picture/59e185c66db4c.jpg') }}"></a>
<div class="list-title"><a target="_blank" href="{{ url('home/postshow')}}?id={{ $v->id }}">{{ $v->title }}</a></div>
<div class="list-desc"><a target="_blank" href="{{ url('home/postshow')}}?id={{ $v->id }}">其实生活，不像京剧里单凭脸谱便知道谁好谁坏、谁善谁恶，善人亦可能大奸；小偷亦有侠义和人性的光芒。</a></div>
<div class="list-author">
<a target="_blank" href="#">{{ $v->type_id }}</a>
<span>{{ $v->created_at }}</span>
<span>点击<a target="_blank" href="http://club.kdnet.net/dispbbs.asp?id=12452989">24204</a></span><span>回复<a target="_blank" href="http://club.kdnet.net/dispbbs.asp?id=12452989">27</a></span><a href="{{ $v->user_id }}" class="plate">来自：{{ $v->nickname }}</a>
</div>
</li>
 @endforeach 
